...
describe("features/recognize", () => {
  ...
  beforeEach(function () {
    ...
    SendNotificationToGiverStub = sinon.stub(
      recognize,
      "SendNotificationToGiver"
    );
    SendNotificationToReceiversStub = sinon.stub(
      recognize,
  ...
  describe.only("Recognize", () => {
    describe("When message is sent with specified emoji", () => {
      it("Adds the emoji to it", async () => {
        ...
        assert.isOk(client.reactions.add.calledOnce);
        assert.equal(
          client.reactions.add.getCall(0).args[0].channel,
          eventObj.channel
        );
        assert.equal(client.reactions.add.getCall(0).args[0].name, "beerjar");
        assert.equal(
          client.reactions.add.getCall(0).args[0].timestamp,
          eventObj.ts
        );
        /* SendNotificationToGiver */
        ...
        assert.equal(
          SendNotificationToReceiversStub.getCall(0).args[0],
          client
        );
        assert.equal(
        ...
        assert.equal(EmojiCountInStub.getCall(0).args[0], eventObj.text);
        /* ReceiverIdsIn */
        assert.isOk(ReceiverIdsInStub.calledOnce);
        assert.equal(ReceiverIdsInStub.getCall(0).args[0], eventObj.text);
      });
    });
  });
  describe.only("Reaction", () => {
    describe("When message is reacted to with specific emoji", () => {
      it("Give beerjar and notify giver/receivers", async () => {
        ...
        assert.equal(GetMessageReactedStub.getCall(0).args[0], client);
        assert.equal(GetMessageReactedStub.getCall(0).args[1], eventObj);
        /* ReceiverIdsIn */
        assert.isOk(ReceiverIdsInStub.calledOnce);
        assert.equal(
          ReceiverIdsInStub.getCall(0).args[0],
          responseMessageObj.messages[0].text
        );
      });
    });
  });
