  ...
  describe("GetMessageReacted", () => {
    describe("When message is reacted to", ()=> {
      ...
      "response_metadata": {
        ...
        assert.isOk(
          client.conversations.replies.calledOnce
        )
        assert.equal(
          message.text, responseObj.messages[0].text
        )
        assert.equal(
          client.conversations.replies.getCall(0).args[0].channel, eventObj.item.channel
        )
        assert.equal(
          client.conversations.replies.getCall(0).args[0].ts, eventObj.item.ts
        )
        assert.equal(
          client.conversations.replies.getCall(0).args[0].limit, 1
        )
      })
    })
  })
  ...
  describe("SendNotificationToReceivers", () => {
    describe("When message contains giver and recipients", ()=> {
      it("Sends ephemeral message as response", async ()=> {
        ...
        assert.isOk(
          client.chat.postMessage.calledTwice
        )

        assert.equal(
          client.chat.postMessage.getCall(0).args[0].channel, discontent.receivers[0]
        )
        assert.equal(
          client.chat.postMessage.getCall(1).args[0].channel, discontent.receivers[1]
        )
        assert.include(
          client.chat.postMessage.getCall(0).args[0].text, getObj.profile.display_name
        )
        assert.include(
          client.chat.postMessage.getCall(0).args[0].text, discontent.count
        )
        assert.equal(
          client.users.profile.get.getCall(1).args[0].user, discontent.receivers[0]
        )
